...
from src.evaluate.action2motion.accuracy import calculate_accuracy
from src.evaluate.action2motion.diversity import calculate_diversity_multimodality
from src.evaluate.action2motion.fid import calculate_fid
from src.evaluate.action2motion.models import load_classifier, load_classifier_for_fid


class A2MEvaluation:
    def __init__(self, dataname, device):
        dataset_opt = {
            "ntu13": {"joints_num": 18, "input_size_raw": 54, "num_classes": 13},
            "humanact12": {"input_size_raw": 72, "joints_num": 24, "num_classes": 12},
        }
        ...
        self.gru_classifier_for_fid = load_classifier_for_fid(
            dataname, self.input_size_raw, self.num_classes, device
        ).eval()
        self.gru_classifier = load_classifier(dataname, self.input_size_raw, self.num_classes, device).eval()
    ...
    def evaluate(self, model, loaders):
        ...
        for key, loader in loaders.items():
            ...
            mkey = f"{metric}_{key}"
            metrics[mkey], _ = calculate_accuracy(model, loader, self.num_classes, self.gru_classifier, self.device)
