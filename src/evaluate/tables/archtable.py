...
def construct_table(folder):
    ...
    model_metrics_dataset = {"ntu13": {}, "uestc": {}}

    epoch_dataset = {"ntu13": 1000, "uestc": 500}
    ...
    ablation_naming = {
        "average_encoder": r"No $\mu_{a}^{token},\Sigma_{a}^{token}$",
...
\end{{document}}
""".format(
        gtrow=gtrow, archrow=groupedrows[0], ablationrow=groupedrows[1]
    )
    return template
