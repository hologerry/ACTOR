...
def add_dataset_options(parser):
    ...
    group.add_argument(
        "--sampling", default="conseq", choices=["conseq", "random_conseq", "random"], help="sampling choices"
    )
    ...
    group.add_argument(
        ...
        type=int,
        nargs="+",
        default=[3.141592653589793, 0, 0],
        help="Default rotation, useful if glob is False",
    )
    group.add_argument(
        "--translation", dest="translation", action="store_true", help="if we want to output translation"
    )
    group.add_argument(
        "--no-translation", dest="translation", action="store_false", help="if we don't want to output translation"
    )
