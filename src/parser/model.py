from src.models.get_model import ARCHINAMES, JOINTSTYPES, LOSSES, MODELTYPES
...
def add_model_options(parser):
    ...
    group.add_argument(
        "--jointstype", default="vertices", choices=JOINTSTYPES, help="Jointstype for training with xyz"
    )
    ...
    group.add_argument(
        ...
        dest="vertstrans",
        action="store_true",
        help="Training with vertex translations in the SMPL mesh",
    )
    group.add_argument(
        "--no-vertstrans",
        dest="vertstrans",
        action="store_false",
        help="Training without vertex translations in the SMPL mesh",
    )
    ...
    group.add_argument(
        "--ablation",
        choices=[None, "average_encoder", "zandtime", "time_encoding", "concat_bias"],
