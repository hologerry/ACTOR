...
def add_visualize_options(parser):
    ...
    group.add_argument("--jointstype", default="smpl", choices=JOINTSTYPES, help="Jointstype for training with xyz")
    group.add_argument(
        "--vertstrans", dest="vertstrans", action="store_true", help="Training with vertex translations"
    )
    group.add_argument(
        "--no-vertstrans", dest="vertstrans", action="store_false", help="Training without vertex translations"
    )
    ...
    group.add_argument("--fact_latent", type=int, default=1, help="factor for max latent space")
