...
def do_epochs(model, datasets, parameters, optimizer, writer):
    ...
    train_iterator = DataLoader(
        train_dataset, batch_size=parameters["batch_size"], shuffle=True, num_workers=8, collate_fn=collate
    )

    if "test" in datasets:
        test_dataset = datasets["test"]
        test_iterator = DataLoader(
            test_dataset, batch_size=parameters["batch_size"], shuffle=False, num_workers=8, collate_fn=collate
        )
