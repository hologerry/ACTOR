...
from .anim import load_anim, plot_3d_motion_dico
...
def generate_by_video(visualization, reconstructions, generation, label_to_action_name, params, nats, nspa, tmp_path):
    ...
    recons = {
        mode: {key: reconstruction[key].data.cpu().numpy() for key in keep}
    ...
    def pool_job_with_desc(pool, iterator, desc, max_, save_path_format, isij):
        ...
        if isij:
            array = np.stack(
                [
                    [load_anim(save_path_format.format(i, j), timesize) for j in range(nats)]
            ...
            array = np.stack(
                [load_anim(save_path_format.format(i), timesize) for i in tqdm(range(nats), desc=desc.format("Load"))]
            )
    ...
    with multiprocessing.Pool() as pool:
        ...
        iterator = (
            (
                ...
                params,
                {"title": f"gen: {label_to_action_name(gener['y'][i, j])}", "interval": 1000 / fps},
            )
            for j in range(nats)
            for i in range(nspa)
        )
        gener["frames"] = pool_job_with_desc(
            pool, iterator, "{} the generated samples", nats * nspa, save_path_format, True
        )
        # Real samples
        ...
        iterator = (
            (
                ...
                params,
                {"title": f"real: {label_to_action_name(visu['y'][i])}", "interval": 1000 / fps},
            )
            for i in range(nats)
        )
        visu["frames"] = pool_job_with_desc(pool, iterator, "{} the real samples", nats, save_path_format, False)
        for mode, recon in recons.items():
            ...
            iterator = (
                (
                    ...
                    save_path_format.format(i),
                    params,
                    {"title": f"recons: {label_to_action_name(recon['y'][i])}", "interval": 1000 / fps},
                )
                for i in range(nats)
            )
            recon["frames"] = pool_job_with_desc(
                pool, iterator, "{} the reconstructed samples", nats, save_path_format, False
            )
...
def viz_epoch(model, dataset, epoch, params, folder, writer=None):
    ...
    visualization = {
        "x": real_samples.to(model.device),
    ...
    with torch.no_grad():
        ...
        if decoder_test == "new":
            ...
            gendurations = torch.einsum("l,k->kl", durations, 1 - convex_factors) + torch.einsum(
                "l,k->kl", durations_below, convex_factors
            )
...
def viz_dataset(dataset, params, folder):
    ...
    figname = "{}_{}_numframes_{}_sampling_{}_step_{}".format(
        params["dataset"], params["pose_rep"], params["num_frames"], params["sampling"], params["sampling_step"]
    )
    ...
    visualization["output_xyz"] = rot2xyz(output.to(device), visualization["mask"].to(device), **rot2xyz_params)
    ...
    with multiprocessing.Pool() as pool:
        ...
        iterator = (
            (
                ...
                save_path_format.format(i, j),
                params,
                {"title": f"real: {label_to_action_name(visu['y'][i, j])}", "interval": 1000 / fps},
            )
            for j in range(nats)
            for i in range(nspa)
        )
        visu["frames"] = pool_job_with_desc(pool, iterator, "{} the real samples", nats, save_path_format)
