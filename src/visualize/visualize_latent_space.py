...
from src.train.trainer import test, train
from src.utils import optutils
from src.visualize import viz_epoch, viz_fake, viz_real
...
if __name__ == "__main__":
    ...
    opt.visualize.update(
        {
            "num_classes": dataset.num_classes,
            "num_actions_to_sample": min(opt.visualize["num_actions_to_sample"], dataset.num_classes),
        }
    )
    ...
    colors = (
        list(mcolors.TABLEAU_COLORS.values()) + list(mcolors.BASE_COLORS.values()) + list(mcolors.CSS4_COLORS.values())
    )
    for label in tqdm(range(num_classes)):
        color = colors[label]
